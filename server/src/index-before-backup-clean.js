...
app.post("/api/admin/backup/create", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
...
app.post("/api/admin/backup/inspect", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const result = await inspectBackupArchive(req.body);

    res.json(result);

  } catch (error) {
    res.status(500).json({
      message: "Backup inspection failed",
      error: error.message
    });
  }
});
...
app.post("/api/admin/backup/restore", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const result = await restoreBackupArchive(req.body);
